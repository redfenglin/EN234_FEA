...
!=========================== ABAQUS format user element subroutine ===================
    ...
    !
      double precision  ::  xi(2,9)                          ! Area integration points for the displacement fields
    ...
    !
      ...
      double precision  ::  eye(4)                              ! I = [1,1,1,0]
      double precision  ::  sol(9), dsol(9)
      double precision  ::  Del(4,4)                          ! stress = D*(strain) for elastic  (NOTE FACTOR OF 2 in shear strain)
      double precision  ::  D(9,9)                            ! New D matrix for this problem
      ...
      double precision  ::  det, detbar                       ! Jacobian determinant
      double precision  ::  f, dfdc                           ! f function and its derivative
      double precision  ::  E, xnu, D44, D11, D12             ! Material properties
      double precision  ::  omega, We, kappa, Dcoef, theta    ! additional material properties
      ...
      call abq_UEL_2D_integrationpoints(n_points, NNODE_Bar,xibar,wbar)
      ...
      do kint = 1, n_points
        ...
        dxidx(2,1:2) =  [-dxdxi(2,1),dxdxi(1,1) ]/det
        dNdx(1:NNODE,1:2) = matmul(dNdxi(1:NNODE,1:2),dxidx)

        call abq_UEL_2D_shapefunctions(xibar(1:2,kint),NNODE_Bar
     1                                                 ,Nbar,dNbardxi)
        dxdxibar = matmul(coords(1:2,1:NNODE_Bar)
     1                                 ,dNbardxi(1:NNODE_Bar,1:2))
        detbar = dxdxibar(1,1)*dxdxibar(2,2)-
     1               dxdxibar(1,2)*dxdxibar(2,1)
        dxidxbar(1,1:2) =  [ dxdxibar(2,2),-dxdxibar(1,2)]/detbar
     ...
     1                                                   ,dxidxbar)
      ...
      ! get a modified B matrtix
        B = 0.d0
      ! B(1:3,1:24) for First DOF: 8 noded quadratic
        B(1,1) = dNdx(1,1)
        B(1,5) = dNdx(2,1)
        B(1,9) = dNdx(3,1)
        B(1,13) = dNdx(4,1)
        ...
        B(3,23) = dNdx(8,2)
        B(3,24) = dNdx(8,1)
      ! B(4:9,1:24) for Second DOF: 4 noded linear
        B(4,3) = Nbar(1)
        B(4,7) = Nbar(2)
        B(4,11) = Nbar(3)
        B(4,15) = Nbar(4)
     ...
     1           ,(strain(1:4)-eye(1:4)*omega*c/3.d0))
        f = 2.d0*We*c*(c-1.d0)*(2.d0*c-1.d0)
        dfdc = We*(12.d0*(c**2)-12.d0*c+2.d0)
      ...
      ! get a q vector
        ...
        q(2) = stress(2)
        q(3) = stress(4)
        q(4) = sol(4)-f-omega*(stress(1)+stress(2)+stress(3))/3.d0
        q(5) = dsol(5)/DTIME
        q(6) = -kappa*sol(8)
        q(7) = -kappa*sol(9)
      ...
      ! get a D matrix
        ...
        D(2,2) = Del(2,2)
        D(3,3) = Del(4,4)
        D(1,5) = -omega*(D11+D12+D12)/3.d0
        D(2,5) = -omega*(D11+D12+D12)/3.d0
        D(4,1) = -omega*(D11+D12+D12)/3.d0
        D(4,2) = -omega*(D11+D12+D12)/3.d0
        D(4,4) = 1.d0
        D(4,5) = -dfdc + (omega**2.d0)*(D11+D11+D11+6.d0*D12)/9.d0
        D(5,5) = 1.d0/DTIME
        D(6,8) = -kappa
        D(7,9) = -kappa
        D(8,6) = theta*Dcoef
     ...
     1     + matmul(transpose(B(1:9,1:24)),matmul(D(1:9,1:9)
     2          ,B(1:9,1:24)))*w(kint)*det
        ...
        if (NSVARS>=n_points*4) then   ! Store stress at each integration point (if space was allocated to do so)
         ...
         ENERGY(2) = ENERGY(2)
     1   + 0.5d0*dot_product(stress,strain)*w(kint)*det
