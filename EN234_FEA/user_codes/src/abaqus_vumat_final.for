...
!
       ...
       double precision sestar,skkstar, sm
       double precision E,xnu,e0,edot0
       double precision T0, T, Tm, Tstar
       double precision A, B, C, n, m
       double precision eplas,deplas, efail
       double precision c1, c2, c3
       double precision D, D1, D2, D3, D4, D5
...
!
     ...
     1                      - sum(strainInc(k,1:ndir))/3.d0
        ...
        skkstar =  E*sum(strainInc(k,1:ndir))/(1.d0-2.d0*xnu)
...
!     Elastic increment (either no stress, or no plastic strain rate)
        ...
        if (sestar/A<1.d-09.or.edot0==0.d0) then
       ...
       if (deplas==0.d0) deplas = 1.d-09/A
       do while (err>tol)
            Tstar = (T-T0)/(Tm-T0)
            c1 = 1.d0+C*log(deplas/dt/edot0)
            c2 = A+B*(eplas**n)
            c3 = 1.d0-Tstar**m
            f = sestar - 1.5d0*E*deplas/(1.d0+xnu) - c1*c2*c3
            dfde = -1.5d0*E/(1.d0+xnu) - C*c2*c3/deplas
        ...
        sm = sum(stressOld(k,1:ndir))/3
        efail = (D1+D2*exp(D3*sm/sestar))*(1+D4*log(deplas/dt/edot0))
     1                       *(1.d0+D5*Tstar)
