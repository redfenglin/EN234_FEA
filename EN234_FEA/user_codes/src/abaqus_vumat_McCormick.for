...
!
       ...
       double precision c1,C,f,dfde,q,tadta
...
!     Elastic increment (either no stress, or no plastic strain rate)
       ...
       do while (err>tol)
         if (deplas>1.d-08) then
            q = Omega/deplas
            tadta = (q*dt+(1+q)*ta*exp(-1.d0/q))/(1+q*exp(-1.d0/q))
            C = 1.d0 - exp(-(tadta/td)**alpha)
            s0 = Y*(1.d0+(eplas+deplas)/e0)**m
            c1 = s0*(eplas+deplas)/S + H*C +log(deplas/dt/edot0)
            f = sestar/S - 1.5d0*E*deplas/(1.d0+xnu)/S - c1
            dfde = -1.5d0*E/(1.d0+xnu)/S - 1.d0/deplas
     &          -s0*(m*(eplas+deplas/(e0+eplas+deplas))+1)/S
     &          +(1.d0-C)*alpha*((tadta/td)**(alpha-1.d0))/td
     &          *ta*exp(-1.d0/q)/(q*(1.d0+q*exp(-1.d0/q))*deplas)
         else
            q = Omega/deplas
            tadta = (q*dt+(1+q)*ta*(1-1.d0/q+0.5*(1.d0/q)**2))
     &                    /(1+q*(1-1.d0/q+0.5*(1.d0/q)**2))
            C = 1.d0 - exp(-(tadta/td)**alpha)
            s0 = Y*(1.d0+(eplas+deplas)/e0)**m
            c1 = s0*(eplas+deplas)/S + H*C +log(deplas/dt/edot0)
            f = sestar/S - 1.5d0*E*deplas/(1.d0+xnu)/S - c1
     ...
     &          *ta*(1-1.d0/q+0.5*(1.d0/q)**2)
     &          /(q*(1.d0+q*(1-1.d0/q+0.5*(1.d0/q)**2))*deplas)/td
         endif
