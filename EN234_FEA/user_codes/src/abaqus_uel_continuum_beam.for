...
!=========================== ABAQUS format user element subroutine ===================
    ...
    !
      ...
      double precision  ::  snodes(2,4), delta1, delta2
      double precision  ::  strainhat(2)
      double precision  ::  ehat1(2), ehat2(2)
      ...
      ! the coordinates of the slave nodes
      ...
      snodes(1,1) = coords(1,1) + h/2.d0*sin1
      snodes(2,1) = coords(2,1) - h/2.d0*cos1
      snodes(1,2) = coords(1,2) + h/2.d0*sin2
      snodes(2,2) = coords(2,2) - h/2.d0*cos2
      snodes(1,3) = coords(1,2) - h/2.d0*sin2
      snodes(2,3) = coords(2,2) + h/2.d0*cos2
      snodes(1,4) = coords(1,1) - h/2.d0*sin1
      snodes(2,4) = coords(2,1) + h/2.d0*cos1
      ...
      Tmat = 0.d0
      Tmat(1,1) = 1.d0
      Tmat(1,3) = coords(2,1) - snodes(2,1)
      Tmat(2,2) = 1.d0
      Tmat(2,3) = -coords(1,1) + snodes(1,1)
      Tmat(3,4) = 1.d0
      Tmat(3,6) = coords(2,2) -snodes(2,2)
      Tmat(4,5) = 1.d0
      Tmat(4,6) = coords(1,2) - snodes(1,2)
      Tmat(5,4) = 1.d0
      Tmat(5,6) = coords(2,2) - snodes(2,3)
      Tmat(6,5) = 1.d0
      Tmat(6,6) = coords(1,2) - snodes(1,3)
      Tmat(7,1) = 1.d0
      Tmat(7,3) = coords(2,1) - snodes(2,4)
      Tmat(8,2) = 1.d0
      Tmat(8,3) = -coords(1,1) + snodes(1,4)


      do kint = 1, n_points
        call abq_UEL_2D_shapefunctions(xi(1:2,kint),4,N,dNdxi)
        dxdxi = matmul(snodes(1:2,1:4),dNdxi(1:4,1:2))
